...
class RequestState:
    ...
    def apply_streaming_update(self, update: StreamingUpdate) -> None:
        ...
        truncation_info = getattr(update, "truncation_info", None)
        is_truncated = (
            truncation_info is not None and truncation_info.get("is_truncated", False)
        )
        if is_truncated:
            if update.prompt:
                self.prompt = update.prompt
            self.prompt_token_ids = list(update.prompt_token_ids) if update.prompt_token_ids else []
