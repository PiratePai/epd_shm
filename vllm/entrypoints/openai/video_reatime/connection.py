...
from vllm.entrypoints.openai.video_reatime.protocol import (
    CompletionDelta,
    CompletionDone,
    ErrorEvent,
    GenerationTrigger,
...
from vllm.entrypoints.openai.video_reatime.serving import (
            ...
            while True:
                ...
                async for output in result_gen:
                    if output.outputs and len(output.outputs) > 0:
                        ...
                        logger.info('output prompt token id len', prompt_token_ids_len)  # this matches with what the scheduler collects
                        ...
                        logger.info('total tokens', len(output.prompt_token_ids), completion_tokens_len)
                        total_tokens = prompt_token_ids_len + completion_tokens_len
                        if total_tokens > self.serving.engine_client.model_config.max_model_len // 2:  # DEFAULT_TOKEN_THRESHOLD:
